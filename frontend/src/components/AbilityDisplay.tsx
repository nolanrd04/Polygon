...
export default function AbilityDisplay({ shieldCharges, hasDash, dashCooldownProgress }: AbilityDisplayProps) {
      ...
      <div className="flex flex-col gap-2">
        ...
        {hasDash && (
          <div className={`bg-gray-900/80 border rounded-lg p-2 min-w-[160px] ${
            dashReady ? 'border-green-500/50' : 'border-blue-500/50'
          }`}>
            <div className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded flex items-center justify-center border ${
                ...
                <span className={`font-bold text-xs ${
                  dashReady ? 'text-green-400' : 'text-blue-400'
                }`}>
                  SPC
                </span>
              </div>
              <div className="flex-1">
                ...
                <div className="w-full h-2 bg-gray-800 rounded-full overflow-hidden mt-0.5 border border-gray-700">
                  <div
                    ...
                    style={{ width: `${dashCooldownProgress * 100}%` }}
                  />
                </div>
              </div>
              <div className={`text-xs font-bold ${
                ...
                {dashReady ? 'RDY' : `${Math.ceil((1 - dashCooldownProgress) * 500)}ms`}
              </div>
            </div>
          </div>
        )}
