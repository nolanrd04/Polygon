...
import { UpgradeTargetID, UpgradeStatID, LightingIntensityID } from '../data/ID'
...
export class Player extends Phaser.GameObjects.Container {
  ...
  /** Dash burst state. Charges, cooldown and the keybind live in
   *  AbilitySystem; what's left here is the movement it drives. */
  ...
  /**
   * Raise the shield, consuming one charge. Returns false when it can't be
   * raised (already up, or no charges left) so AbilitySystem knows the
   * activation was declined.
   */
  ...
  update(): void {
    ...
    LightingSystem.AddLight(this.x, this.y, COLORS.player, LightingIntensityID.Player)
  ...
  /**
   * Fire off a dash burst in the direction of travel (or of facing, when
   * standing still). Ownership and charge availability are AbilitySystem's
   * job — by the time this runs, the dash is paid for.
   */
  performDash(): boolean {
    const now = this.scene.time.now
